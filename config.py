...
MAX_PREVIEW_TEXT_LENGTH = int(os.getenv("MAX_PREVIEW_TEXT_LENGTH", 30)) # Максимальная длина текста заказа для предпросмотра
...
# Формат: начинается с '+', затем 1-3 цифры (код страны), затем 7-15 цифр.
# Это более гибкий вариант, который покрывает большинство международных номеров.
# Примеры: +380991234567, +12125550123
PHONE_NUMBER_REGEX = re.compile(r"^\+\d{1,3}\d{7,15}$")
...
ORDER_FIELDS_CONFIG = [
    ...
    },
    {
        ...
        "prompt": (
            ...
            "Формат: `+<код страны><номер>` (например, `+380991234567` или `+12125550123`)."
        ),
